  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      if (!profile?.is_active) {
        // Redirect to activation page if user is not active
        router.push(`/client-activation${redirectUri ? `?redirect=${encodeURIComponent(redirectUri)}` : ''}`)
      } else {
        // Success - If redirectUri exists (desktop app context), we can handle it
        // but for now, we just go to a success state or dashboard
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f8] flex items-center justify-center px-4">
      <form onSubmit={handleLogin} className="w-full max-w-md bg-white p-8 rounded-lg border border-[#d8dee6] shadow-sm">
        ...
        <p className="text-sm text-gray-600 mb-6">Sign in with your email and password.</p>
